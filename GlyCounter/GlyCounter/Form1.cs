...
namespace GlyCounter
{
    public partial class Form1 : Form
    {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    if (!_csvCustomFile.Equals("empty"))
                    {
                        ...
                        {
                            OxoniumIon oxoIon = new OxoniumIon { theoMZ = double.Parse(csv["m/z"]) };
                    ...
                    foreach (string fileName in _fileList)
                    {
                        ...
                        for (int i = rawFile.FirstScan; i <= rawFile.LastScan; i++)
                        {
                            ...
                            if (spectrum.MsLevel == 2)
                            {
                                ...
                                bool etdTrue = false;
                                bool uvpdTrue = false;
                                List<double> oxoniumIonFoundPeaks = [];
                                List<double> oxoniumIonFoundMassErrors = [];
        ...
        public OxoniumIon ProcessOxoIon(object item, string glycanSource, bool check204 = false)
        {
            ...
            {
                theoMZ = Convert.ToDouble(oxoniumIonArray[0]), description = item.ToString(), glycanSource = glycanSource,
                hcdCount = 0,
                etdCount = 0,
                uvpdCount = 0,
                ...
                await Task.Run(() =>
                {
                    ...
                    foreach (PSM psm in psmList)
                    {
                        ...
                        if (spectrum.MsLevel == 2 && !spectrum.Analyzer.Contains("ITMS") && spectrum.TotalIonCurrent > 0)
                        {
                            ...
                            List<SpecDataPointEx> yIonFoundPeaks = [];
