...
namespace GlyCounter
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                textBox1.Text = @"Successfully uploaded " + fdlg.FileNames.Length + @" file(s)";
        ...
        private async void StartButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (string fileName in _fileList)
                    {
                        ...
                        int numberScansCountedLikelyGlyco_total;
                        ...
                        bool likelyGlycoSpectrum;
                        double nce = 0.0;
                        double halfTotalList = _oxoniumIonHashSet.Count / 2.0;
            ...
            {
                timer1.Stop();
                StatusLabel.Text = "Output to: " + _outputPath;
        ...
        public OxoniumIon ProcessOxoIon(object item, string glycanSource, bool check204 = false)
        {
            ...
            {
                theoMZ = Convert.ToDouble(oxoniumIonArray[0], CultureInfo.InvariantCulture),
                description = item.ToString(),
                glycanSource = glycanSource,
                hcdCount = 0,
                etdCount = 0,
                uvpdCount = 0,
                peakDepth = ArbitraryPeakDepthIfNotFound
                ...
                await Task.Run(() =>
                {
                    ...
                    if (string.IsNullOrEmpty(_outputPath) || !Directory.Exists(_outputPath))
                    {
                        if (!string.IsNullOrEmpty(_Ynaught_rawFilePath) && File.Exists(_Ynaught_rawFilePath))
                        {
                            ...
                            {
                                Ynaught_outputTextBox.Text = _outputPath;
                            }
                        }
                        else
                        {
                    ...
                    foreach (object? item in Yions_NlinkedCheckBox.CheckedItems)
                    {
                        ...
                        Yion yIon = new Yion
                        {
                            theoMass = Convert.ToDouble(yIonArray[0], CultureInfo.InvariantCulture),
                            description = item.ToString(),
                    ...
                    foreach (object? item in Yions_OlinkedChecklistBox.CheckedItems)
                    {
                        string[] yIonArray = item.ToString().Split(',');
                        Yion yIon = new Yion
                        {
                    ...
                    foreach (object? item in Yions_LossFromPepChecklistBox.CheckedItems)
                    {
                        ...
                        Yion yIon = new Yion
                        {
                            theoMass = Convert.ToDouble(yIonArray[1].Substring(1), CultureInfo.InvariantCulture),
                            description = item.ToString(),
                    ...
                    if (_ipsa)
                    {
                        outputIPSA = new StreamWriter(Path.Combine(_outputPath + @"\" + fileNameShort + "_Glycounter_YionIPSA.txt"));
                    ...
                    foreach (PSM psm in psmList)
                    {
                        ...
                        if (spectrum.MsLevel == 2 && !spectrum.Analyzer.Contains("ITMS") && spectrum.TotalIonCurrent > 0)
                        {
                            ...
                            //look for each Y-ion
                            List<Yion> finalYionList = []; //creating this to store charge separately

                            foreach (Yion yIon in _yIonHashSet)
                            {
                                Yion foundYion = new Yion
                                {
                                    description = yIon.description, theoMass = yIon.theoMass, glycanSource = yIon.glycanSource,
                                    //I think these are not necessary to update but just in case
                                    hcdCount = yIon.hcdCount,
                                    etdCount = yIon.etdCount
                                };
