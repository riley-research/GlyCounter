...
namespace GlyCounter
{
    public class iC_ProcessTimsTOF
    {
        ...
        public static async Task<(GlyCounterSettings, RawFileInfo)> iC_processTimsTOFAsync(
            string fileName, GlyCounterSettings glySettings, iCounterSettings iCsettings,
                ...
                try
                {
                    await foreach (var msg in writeChannel.Reader.ReadAllAsync(token).ConfigureAwait(false))
                    {
                        if (msg.OxoLine is not null)
                            await outputSignal.WriteLineAsync(msg.OxoLine).ConfigureAwait(false);
                    ...
                    try
                    {
                        await foreach (var spectrum in spectraChannel.Reader.ReadAllAsync(token).ConfigureAwait(false))
                        {
                            ...
                            if (basePeak > 0)
                            {
                                ...
                                foreach (var local in localIons)
                                {
